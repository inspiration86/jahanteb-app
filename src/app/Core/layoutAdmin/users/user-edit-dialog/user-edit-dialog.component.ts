...
export class UserEditDialogComponent implements OnInit {
  ...
  errorMessages = {
    email: [
      {
        type: 'maxlength',
        message: 'پست الکترونیکی نمی تواند بیشتر از 200 کاراکتر باشد.',
      },
  ...
  ngOnInit(): void {
    ...
    if (this.user.hasOwnProperty('country')) {
      if (this.user.country != null) {
        ...
        if (this.user.hasOwnProperty('state')) {
          if (this.user.state != null) {
            this.countryOnChange(this.selectedCountry.code);
            this.selectedState = this.states.filter(
              (x) => x.name == this.user.state
            )[0];
  ...
  createform(): void {
    this.form = this.formBuilder.group({
      ...
      nationalCode: new FormControl(this.user.nationalCode, [
  ...
  submitForm(): void {
    ...
    this.service
      .editUser(this.user._id, this.form.value)
      .subscribe((response) => {
